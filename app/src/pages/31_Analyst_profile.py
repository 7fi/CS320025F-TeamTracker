...
coachInfo = requests.get(f"http://web-api:4000/analysts/{st.session_state['selected_ID']}")
coachInfo = coachInfo.json()
logger.info(coachInfo)

if st.session_state['role'] == 'analyst' and st.session_state['selected_ID'] != st.session_state['userID']:
